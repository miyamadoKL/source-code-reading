...
各 `Task` は状態機械（state machine）を持ち、`New` → `Scheduling` → `Bound` → `Completed` のように状態が遷移する。
...
6. **Pod バインド**: `AsyncRMCallback` が割り当て決定を受け取り、`Dispatcher` を通じて `Task` を `Bound` 状態に遷移させる。`Context` は Kubernetes API Server に Pod のバインドを指示する。
7. **完了通知**: Pod が正常に起動したら、`Task` は `Completed` 状態に遷移し、core に完了を通知する。
