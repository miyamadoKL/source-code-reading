...
[staging/src/k8s.io/apiserver/pkg/storage/cacher/cacher.go L929-L940](https://github.com/kubernetes/kubernetes/blob/v1.36.2/staging/src/k8s.io/apiserver/pkg/storage/cacher/cacher.go#L929-L940) で、Cacher は約1秒間隔で Bookmark を生成する。
...
`defaultBookmarkFrequency`（=1分）に加えて、タイマーは1秒間隔でも Bookmark を送る。
これは `dispatchEvents` のループで、`bookmarkTimer` が `Jitter(time.Second, 0.25)` でリセットされるためである。
