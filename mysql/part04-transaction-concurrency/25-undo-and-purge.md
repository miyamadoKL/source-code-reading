...
第24章では、リードビューが各レコードの `DB_TRX_ID` を見て、自分に見える版だけを選び出す仕組みを読んだ。
...
クラスタ化インデックスのレコードが、ユーザ列のほかに `DB_TRX_ID`（最後に更新したトランザクションの ID）と `DB_ROLL_PTR`（直前の版を指すロールポインタ）という2つのシステム列を持つことは、第14章と第24章で見た。
本章ではこの `DB_ROLL_PTR` を起点に話を進める。
リードビューの可視判定そのものは第24章で扱ったので、ここでは「見えない版を見つけたあと、どうやって古い版へ降りるか」に焦点を絞る。
...
第24章のリードビューは、レコードの `DB_TRX_ID` が見えないと判定したとき、この `trx_undo_prev_version_build` を繰り返し呼んで、自分に見える版へ降りていく。
...
- [第14章 ページとレコードのフォーマット](../part02-innodb-foundation/14-page-and-record-format.md)：`DB_TRX_ID` と `DB_ROLL_PTR` のシステム列がレコードのどこに置かれるか。
- [第23章 トランザクション管理](23-transaction-management.md)：rseg の割り当てとコミットの流れ。
- [第24章 MVCC とリードビュー](24-mvcc-and-read-view.md)：リードビューの可視判定と、本章の版の鎖をたどる呼び出し元。
- [第26章 ロック](26-locking.md)：削除マークと回収のあいだの並行制御。
