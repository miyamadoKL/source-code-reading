...
第16章で読んだミニトランザクション（mtr）は、複数ページへの一続きの変更を不可分にまとめる低い層の単位だった。
...
リードビューはこの `rw_trx_ids` のスナップショットから作られる（第24章）。
コミット時に `trx_t` が `trx_sys` からどう外れ、undo がどう確定するかまでを追う。

MVCC とリードビューの中身は第24章、undo ログとパージは第25章、ロックは第26章で読む。
本章は、その土台となるトランザクションの一生に絞る。

## 前提

第16章で、InnoDB のページ変更は mtr 単位で原子的に適用され、コミット時に redo ログへ書き出されることを読んだ。
...
その undo の確定と、パージ（不要になった undo の回収）の詳細は第25章で読む。
...
リードビューを作るとき、この `rw_trx_ids` のスナップショットを取る（第24章）。
...
durable にするための redo フラッシュとグループコミットの詳細は第27章で読む。
...
- [第16章 ミニトランザクション](../part02-innodb-foundation/16-mini-transaction.md)：本章のコミットが内部で起こす mtr の仕組み。
- [第24章 MVCC とリードビュー](24-mvcc-and-read-view.md)：`rw_trx_ids` のスナップショットからリードビューを作り、可視性を決める。
- [第25章 undo ログとパージ](25-undo-and-purge.md)：本章で確定した undo を `no` の順に回収する。
- [第26章 ロック](26-locking.md)：コミット時に解放するレコードロックと暗黙ロックの実装。
