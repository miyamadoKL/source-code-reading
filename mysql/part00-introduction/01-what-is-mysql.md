...
本章で出てくる用語のうち、パーサ、オプティマイザ、エグゼキュータ、トランザクションといった語は、それぞれ専用の章で改めて掘り下げる。
...
MyISAM などの旧来のエンジンは、トランザクションやクラッシュリカバリを持たず現代の用途では主流でないため、本書では `handler` の別実装として軽く触れるにとどめる。
