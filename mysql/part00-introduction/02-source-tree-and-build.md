...
| ディレクトリ | 役割 | 本書での扱い |
| --- | --- | --- |
| `sql/` | サーバ層であり、接続管理、パーサ、オプティマイザ、エグゼキュータ、`handler` インターフェースを定義する | 第3章から第15章 |
| `sql-common/`、`include/` | サーバとクライアントが共有する宣言、共通ユーティリティのヘッダ | 必要箇所で参照 |
| `mysys/`、`strings/`、`vio/` | OS 抽象、文字列処理、ネットワーク入出力の低レベルライブラリ | 必要箇所で参照 |
| `storage/innobase/` | ストレージエンジン InnoDB の実装 | 第17章以降の主対象 |
| `storage/` 直下の他ディレクトリ | MyISAM、CSV、MEMORY などの他ストレージエンジン | 第39章で概観 |
| `libbinlogevents/` | binlog イベントのフォーマット定義 | 第37章で参照 |
...
本書で詳しく読む主なサブディレクトリを次に挙げる。

| サブディレクトリ | 役割 | 本書での扱い |
| --- | --- | --- |
| `buf/` | バッファプール、すなわちディスクページのメモリキャッシュ | 第20章 |
| `btr/` | B+tree によるインデックスの探索と更新 | 第22章、第23章 |
| `trx/` | トランザクション管理、MVCC、パージ | 第28章から第30章 |
| `lock/` | レコードロックとギャップロックを管理するロックマネージャ | 第31章 |
| `log/` | redo ログ、チェックポイント | 第32章、第34章 |
| `dict/` | テーブルやインデックスのメタデータを保持するデータディクショナリ | 第35章 |
| `row/` | 行レベルの挿入、更新、削除の操作 | 第24章 |
| `fil/` | テーブルスペースのファイル空間管理 | 第18章 |
| `ibuf/` | セカンダリインデックス更新を遅延適用するチェンジバッファ | 第25章 |
| `lob/` | ページに収まらない大きな値を格納する LOB | 第27章 |
| `mtr/` | ページ更新と redo の単位であるミニトランザクション | 第21章 |
| `page/`、`rem/` | ページとレコードのフォーマット | 第19章 |
| `fsp/`、`fut/` | ファイルスペースとセグメントの割り当て | 第18章 |
...
ここで関数名の地図を持っておくと、第5章以降の各章が縦の流れのどこを担うかを位置づけられる。
...
パーサの内部、すなわちトークン化と文法規則による構文木の構築は第5章で読む。
...
イテレータによる実行モデルは第13章と第14章で読む。
イテレータが行を1つ取り出すたびに、対象テーブルの `handler` を通じてストレージエンジンへ要求が下りる。
この `handler` インターフェースこそがサーバ層と InnoDB を分ける境界であり、第15章で詳しく読む。
...
- [第9章 オプティマイザ（論理変換とクエリブロック）](../part01-sql-layer/09-optimizer-transformations.md)
...
- [第17章 InnoDB アーキテクチャ概観](../part02-innodb-foundation/17-innodb-architecture.md)
