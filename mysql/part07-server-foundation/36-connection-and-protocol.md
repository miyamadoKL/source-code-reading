# 第36章 接続の確立と認証、Classic Protocol
...
スクランブルの先回り送付と応答キャッシュの二つを合わせると、クライアントが既定プラグインを使う通常のログインは、握手とクライアント応答の1往復で認証まで届く。
プラグインが食い違うときだけ追加の1往復を払う。
よくある場合を速く、まれな場合は正しく、という設計になっている。
...
スクランブルを先回りで握手に同梱し、クライアントの応答をキャッシュして再利用することで、通常のログインを1往復に収め、プラグインが食い違うときだけ追加の往復を払う。
...
- [第37章 MEM_ROOT と文単位のメモリ寿命](37-mem-root.md)
