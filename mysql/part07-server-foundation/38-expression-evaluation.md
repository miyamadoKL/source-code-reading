...
第9章で読んだエグゼキュータの `FilterIterator` は、`WHERE` 条件の評価に `m_condition->val_int()` を呼んでいた。
その呼び出しの先で何が起きるのかを、本章で具体化する。

## 前提

- 第4章のパーサが式を `Item` の木として組み立て、第5章の準備（`fix_fields`）が列参照を解決し型を確定させた状態から始める。
- 第9章のとおり、エグゼキュータはプルモデルで1行ずつ進み、行ごとに `WHERE` 条件の `Item` を評価する。
...
この `Field` がどの行を指すかは、第9章のイテレータが `handler` を呼んでレコードバッファ（`table->record[0]`）を満たすことで切り替わる。
...
`fix_fields` は第5章の準備の中核であり、列参照の解決、結果型の確定、NULL になりうるかの判定をまとめて行う。
...
第9章で読んだ `FilterIterator::Read` は、まさにこの条件 `Item` の `val_int` を1行ごとに呼んでいた。
...
- [第5章 クエリの解決と準備](../part01-sql-layer/05-resolution-and-prepare.md)
