...
第15章で、SQL 層はストレージエンジンを `handler` という抽象クラス越しに呼ぶことを読んだ。
第17章以降は、その `handler` を実装する一エンジン InnoDB を中核として深く読んできた。
本章は最後に視点を戻し、InnoDB 以外のエンジンが同じ `handler` をどう実装しているかを軽く読む。

対象は **MyISAM**、**MEMORY**（コード上は HEAP）、**CSV**（コード上は TINA）、**FEDERATED** の4つである。
いずれも `handler` を継承し、第15章で読んだ `write_row` や `rnd_next` といった同じ仮想メソッドを埋める。
...
第15章で、エンジン1種類につき1個の関数表 `handlerton` と、開いているテーブル1つにつき1個の `handler` という2層構造を読んだ。
...
データとインデックスを別ファイルに分ける構成は、1つのテーブルスペース内にクラスタ化インデックスとして行を畳み込む InnoDB（第18章、第19章）と対照的である。
...
InnoDB がレコードロックやギャップロック（第31章）でテーブル内の特定の行だけをロックするのに対し、MyISAM が差し出すロックの粒度はテーブル全体である。
...
ここには、InnoDB の `write_row`（第15章）が経由した redo ログへの記録やミニトランザクション（第21章）に当たる処理がない。
`mi_write` が `.MYD` と `.MYI` へ書くだけで、変更前の状態を残す undo ログ（第30章）も、書き込みをアトミックにまとめる仕組みもない。
...
InnoDB の `innodb_init`（第15章）が `commit`、`rollback`、`prepare`、`recover` などトランザクションと XA リカバリの関数ポインタを次々に埋めていたのと比べると、ここには `commit` も `recover` もない。
...
FEDERATED は、ここまでの3エンジンとは成り立ちが違う。
...
ここまでの3エンジンは、第15章で読んだ `handler` の同じ仮想メソッド（`write_row`、`rnd_next`、`store_lock` など）を実装する。
...
本書はサーバ層をたどったあと、InnoDB の実装を中心に詳しく読んできた。
最後にその選択の理由を一段落で振り返る。
InnoDB が既定のエンジンになったのは、耐久性と並行性を機構として備えるからである。
変更を redo ログとミニトランザクションで原子的に永続化し（第21章、第32章）、クラッシュ後はその redo を再適用して一貫した状態へ戻す（第34章）。
読み取りは MVCC とリードビューでロックを取らずに過去のバージョンを見せ（第29章）、書き込みは行単位のレコードロックとネクストキーロックで競合を最小化する（第31章）。
本章で読んだ3エンジンが捨てているこれらの機構こそ、多くのセッションが同じテーブルを安全に読み書きし、障害後もデータを失わないという、汎用データベースに求められる性質を支えている。

## 関連する章

- [第15章 ハンドラ API とストレージエンジンプラグイン](../part01-sql-layer/15-handler-api.md)：本章の3エンジンが実装する `handler` と `handlerton` の2層構造を読む。
- [第31章 ロック](../part04-transaction-concurrency/31-locking.md)：MyISAM のテーブルロックと対比される InnoDB の行単位ロックを読む。
- [第34章 チェックポイントとクラッシュリカバリ](../part05-log-recovery/34-checkpoint-and-recovery.md)：本章の3エンジンが持たないクラッシュリカバリを InnoDB がどう実装するかを読む。
