...
第17章の B+tree は `dict_index_t` を、第19章の行操作は `dict_table_t` を当然のように受け取っていた。
...
第11章でハンドラ API を、第13章でテーブルスペースとファイル空間管理を読んだ。
DD テーブルもまた InnoDB のテーブルであり、`mysql` テーブルスペースに B+tree として格納される。
本章で扱う `dict_table_t` と `dict_index_t` は、第17章と第19章ですでに使ってきた構造体である。
ここではその構造体の中身と、生成経路を読む。

DDL がこれらの定義をどう書き換えるか（オンライン DDL、インスタント DDL）は第31章に送る。
...
第17章でルートから葉へ降りたとき、出発点になったのがこの値である。
...
DD テーブルは `mysql` テーブルスペースに置かれた B+tree なので、定義を読むには第18章のカーソルで木を降り、ページを読む必要がある。
一度開いたテーブルの定義を `dict_table_t` としてメモリに残しておけば、次回は `table_hash` か `table_id_hash` のハッシュ検索一回で同じ構造体に到達し、この木の探索をまるごと省ける。
これは第15章のバッファプールがページを常駐させるのと同じ発想を、定義オブジェクトの層で行うものである。
...
- [第11章 ハンドラ API とストレージエンジンプラグイン](../part01-sql-layer/11-handler-api.md)：DD がまたがるサーバ層とストレージエンジンの境界を扱う。
- [第13章 テーブルスペースとファイル空間管理](../part02-innodb-foundation/13-tablespace-and-fsp.md)：DD テーブルが格納される `mysql` テーブルスペースの管理を扱う。
- [第17章 B+tree インデックス](../part03-index-row/17-btree-index.md)：`dict_index_t` が指す根ページから始まる木構造を扱う。
- [第18章 レコード検索とカーソル](../part03-index-row/18-search-and-cursor.md)：DD テーブルから定義を読むときに使う木の降下を扱う。
- [第31章 オンライン DDL とインスタント DDL](31-online-and-instant-ddl.md)：本章で読んだ定義を変更する側の処理を扱う。
