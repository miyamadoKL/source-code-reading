...
ハンドラ API の枠組みは[第15章 ハンドラ API とストレージエンジンプラグイン](../part01-sql-layer/15-handler-api.md)で扱った。
計測対象として本章が例にとるミューテックスは、InnoDB をはじめサーバ全体で使われる排他制御の基本部品であり、[第21章 ミニトランザクション](../part02-innodb-foundation/21-mini-transaction.md)や[第31章 ロック](../part04-transaction-concurrency/31-locking.md)で触れたラッチ操作の下層にある。
...
- [第15章 ハンドラ API とストレージエンジンプラグイン](../part01-sql-layer/15-handler-api.md)：Performance Schema が `handler` として計測値を表に見せる土台。
- [第21章 ミニトランザクション](../part02-innodb-foundation/21-mini-transaction.md)：計測対象となるラッチ操作の典型。
- [第31章 ロック](../part04-transaction-concurrency/31-locking.md)：ミューテックス待ちが観測される並行制御の文脈。
