...
[第13章 エグゼキュータ（イテレータ実行モデル）](13-executor-iterators.md) のイテレータインタフェース（`Init()`／`Read()`／戻り値の規約）を前提とする。
結合順序やどのアクセスパスを選ぶかの決定は [第10章](10-optimizer-join-cost.md) と [第11章](11-optimizer-access-paths.md) のオプティマイザが済ませており、本章はその決定にもとづいて組み立てられたイテレータ木を実行する側を読む。
...
ビルド入力とプローブ入力は、「同じ結合キーのハッシュで同じ番号のチャンクへ」振り分けられる。
...
オプティマイザは [第10章](10-optimizer-join-cost.md) のコストモデルにもとづいて、結合ごとにどちらのイテレータを使うかを決める。
...
- [第13章 エグゼキュータ（イテレータ実行モデル）](13-executor-iterators.md)：本章の複合イテレータが従う `Init()`／`Read()` の規約。
- [第10章 オプティマイザ（join 順序とコストモデル）](10-optimizer-join-cost.md)：ハッシュ結合とネステッドループの選択を決めるコストモデル。
- [第11章 オプティマイザ（アクセスパスと range optimizer）](11-optimizer-access-paths.md)：ソートや結合の前段に置かれるアクセスパスの決定。
