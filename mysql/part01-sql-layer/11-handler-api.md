...
第9章と第10章で読んだエグゼキュータは、行を1件ずつ取り出して結合やソートに流すイテレータの連なりだった。
...
第9章で、エグゼキュータがイテレータの `Read` を連鎖的に呼んで行を1件ずつ引き出すモデルを読んだ。
...
本章はこの変換の入口までを扱い、InnoDB の行フォーマットそのものは第14章で読む。
...
```mermaid
flowchart TD
    iter["TableScanIterator::Read（第9章）"]
...
引数 `record` のコメントが「a row in MySQL format」と明記しているとおり、ここに渡るのは第10章までの SQL 層が扱う共通の行イメージである。
InnoDB はこれを内部表現に変換してクラスタ化インデックスへ挿入する。
変換と挿入の詳細は第14章（ページとレコードのフォーマット）と第19章（行の挿入、更新、削除）で読む。
...
`row_search_mvcc` がインデックスのどこをどう辿り、どのバージョンの行を可視と判断するかは、第18章（レコード検索とカーソル）と第24章（MVCC とリードビュー）で読む。
...
- [第9章 エグゼキュータ（イテレータ実行モデル）](09-executor-iterators.md)：本章のラッパーを呼ぶイテレータ `Read` の連鎖を読む。
- [第14章 ページとレコードのフォーマット](../part02-innodb-foundation/14-page-and-record-format.md)：`write_row` が行を詰め替える先の InnoDB レコード形式を読む。
- [第18章 レコード検索とカーソル](../part03-index-row/18-search-and-cursor.md)：`index_read` が呼ぶ `row_search_mvcc` の B+tree 検索を読む。
- [第34章 他のストレージエンジン](../part06-dictionary-ddl-ops/34-other-storage-engines.md)：`handler` の別実装として MyISAM などを読む。
