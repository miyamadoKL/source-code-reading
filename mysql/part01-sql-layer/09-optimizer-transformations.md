...
第5章でパーサが `LEX` を構築し、第7章で名前解決と準備（`Query_block::prepare`）が走ることを見た。
...
変換後、内側テーブルは親クエリブロックの一部となり、第10章で扱う join 順序探索の対象に乗る。
...
flowchart TD
    ...
    search["join 順序とコスト探索<br/>(第10章へ)"]
    access["アクセスパス選択<br/>(第11章へ)"]
...
- [第7章 クエリの解決と準備](07-resolution-and-prepare.md)：本章の構造変換が走る `Query_block::prepare` の全体。
- [第10章 オプティマイザ（join 順序とコストモデル）](10-optimizer-join-cost.md)：準結合へ平坦化されたテーブルを含む結合順序探索。
- [第11章 オプティマイザ（アクセスパスと range optimizer）](11-optimizer-access-paths.md)：正規化された条件を使うアクセスパス選択。
- [第13章 エグゼキュータ（イテレータ実行モデル）](13-executor-iterators.md)：実体化や IN→EXISTS のサブクエリ実行。
