...
1組のチャンクペアはメモリに収まる大きさに分割されているので、各ペアの結合はメモリ内ハッシュ結合として完結する。
...
この機構がなぜ効くかを一文で言えば、結合キーのハッシュで両入力を同じ番号のチャンクへ振り分けることで、巨大な結合を「メモリに収まる独立した小さな結合」の集まりに分解できるからである。
...
`join_buffer_size` を超えると、結合キーのハッシュで両入力を同じ番号のチャンクペアへ振り分け、巨大な結合をメモリに収まる小さな結合の集まりへ分解してディスクへスピルする。
...
3つに共通するのは、メモリに収まる場合はメモリ内で完結し、超えた場合だけディスクへスピルする二段構えである。
