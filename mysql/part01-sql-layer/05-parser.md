...
木の構築（本章）と、木から実行コマンドへの変換（第7章）が、この `make_cmd` の境目で分かれている。
...
flowchart TD
    ...
    lex --> qb
    action --> root
    root -.->|"第7章: make_cmd で Sql_cmd へ"| lex
```
...
名前解決と型検査は、第7章で読む `make_cmd` 以降の文脈づけ（`do_contextualize`）に集約される。
...
- 第7章「[クエリの解決と準備](07-resolution-and-prepare.md)」では、本章で作った木に対する `make_cmd` 以降の意味解析（名前解決と型検査）を読む。
- 第6章「[MEM_ROOT と文単位のメモリ寿命](06-mem-root.md)」では、本章のパースツリーが載る `mem_root` の仕組みと文単位のメモリ寿命を読む。
