...
第5章で、パーサがパースツリーのノードを `thd->mem_root` 上に確保することを見た。
本章は、その `mem_root` の実体と寿命管理を詳しく読む位置にある。
...
1.5倍ずつ広げると、確保量に比例して `malloc` を繰り返す事態を避けられ、1回の確保あたりの償却コストを小さく保てる。
...
確保の速い経路は比較1つとポインタ加算1つで終わり、ブロックサイズを1.5倍ずつ広げることで確保量に比例した `malloc` の繰り返しを避ける。
...
- [第5章 パーサ](05-parser.md)：パースツリーのノードを `mem_root` 上に確保する `NEW_PTN` と、パース前後の `mem_root` への上限設定を扱う。
