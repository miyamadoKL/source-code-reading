...
第8章までで、オプティマイザはクエリの実行手順を**アクセスパス**（`AccessPath`）の木として組み立てた。
...
第8章で読んだとおり、最適化の結果は `AccessPath` の木として表現される。
...
- [第8章 オプティマイザ（アクセスパスと range optimizer）](08-optimizer-access-paths.md)
- [第10章 エグゼキュータ（結合、ソート、集約）](10-executor-join-sort-agg.md)
- [第11章 ハンドラ API とストレージエンジンプラグイン](11-handler-api.md)
- [第2章 ソースツリーとビルド、クエリ処理の俯瞰](../part00-introduction/02-source-tree-and-build.md)
- [第38章 式評価（Item の実行時モデル）](../part07-server-foundation/38-expression-evaluation.md)
