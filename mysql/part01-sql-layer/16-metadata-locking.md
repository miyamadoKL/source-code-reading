...
InnoDB の行ロック（第31章）は、トランザクションが触る個々の行を、ストレージエンジンの内部で保護する。
...
第36章で読む DDL は、テーブル定義を書き換える前に MDL の排他ロックを取り、書き換えが終わるまで保持する。
...
書きの DML どうしを実際にどう調停するかは、第31章で読む InnoDB の行ロックが下の階層で受け持つ。
