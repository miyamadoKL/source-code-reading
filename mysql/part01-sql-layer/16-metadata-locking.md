...
第31章で読んだ InnoDB の行ロックは、トランザクションが触る個々の行を、ストレージエンジンの内部で保護していた。
...
第36章で読んだ DDL は、テーブル定義を書き換える前に MDL の排他ロックを取り、書き換えが終わるまで保持する。
本章は、その DDL と、第13章以降で読んだ DML の実行とが、テーブルオープンの時点でどう調停されるかを下から読む。
...
第13章以降で読んだ実行は、その前段でテーブルを開き、定義をテーブル定義キャッシュから取る。
...
書きの DML どうしを実際にどう調停するかは、第31章で読んだ InnoDB の行ロックが下の階層で受け持つ。
...
- [第31章 ロック](../part04-transaction-concurrency/31-locking.md)：MDL の下の階層で、行の競合を調停する InnoDB の行ロック。
- [第36章 オンライン DDL とインスタント DDL](../part06-dictionary-ddl-ops/36-online-and-instant-ddl.md)：MDL の排他ロックを取って定義を書き換える DDL の実装。
- [第35章 データディクショナリ](../part06-dictionary-ddl-ops/35-data-dictionary.md)：MDL が保護するテーブル定義の格納先。
