...
第19章で、16KB の INDEX ページが内部にレコードをキー順の一方向リストとして並べ、ページディレクトリで対数時間の二分探索を可能にすることを読んだ。
...
第18章でテーブルスペースとファイル空間管理を、第19章でページとレコードのフォーマットを読んだ。
本章はそれらのページを節点として扱い、ページ間のリンクをたどる。
ページ内のレコード走査やページディレクトリの二分探索は第19章の範囲とし、本章では木をまたぐ部分に絞る。

木をまたぐ検索（ルートから目的の葉へ降りるカーソル）の詳細は第23章へ送る。
...
ただし葉レコードに収まらない大きな値はページ外へ追い出され、葉には20バイトの参照だけが残る（第27章）。
...
木の各段を区別するのに使われるのが、第19章で見た `PAGE_LEVEL` である。
...
ノードポインタのレコード種別には `REC_STATUS_NODE_PTR` が立ち、第19章で見たレコードヘッダの3ビット種別が001になる。
比較に使うフィールド数を `n_fields_cmp` でページ番号フィールドの手前までに制限しているのは、上位の段では同じキーで別ページを指すノードポインタが共存しうるため、ページ番号で比較してはならないからである。

同じ段に並ぶページは、第19章で見た `FIL_PAGE_PREV` と `FIL_PAGE_NEXT` で双方向リストにつながれている。
...
この判定は、第19章で見たページヘッダの `PAGE_LAST_INSERT`（直前に挿入したレコードの位置）を手がかりにする。
...
左右の隣ページ番号は、第19章で見た `FIL_PAGE_PREV` と `FIL_PAGE_NEXT` から得る。
...
- [第19章 ページとレコードのフォーマット](../part02-innodb-foundation/19-page-and-record-format.md)：本章のページ1枚の内部構造と、ページディレクトリによるページ内二分探索を扱う。
- [第21章 ミニトランザクション](../part02-innodb-foundation/21-mini-transaction.md)：本章の分割やマージを1つの原子的な単位にまとめ、redo ログで保護する仕組みを扱う。
- [第23章 レコード検索とカーソル](23-search-and-cursor.md)：本章の木をルートから葉へ降りる探索と、葉のリンクをたどるカーソル移動を扱う。
- [第24章 行の挿入、更新、削除](24-row-dml.md)：本章の楽観的挿入と悲観的挿入を、行 DML の処理全体の中に位置づける。
- [第26章 アダプティブハッシュインデックス](26-adaptive-hash-index.md)：本章の分割ヒューリスティックが1件残す理由となる、ページ内探索位置のハッシュキャッシュを扱う。
