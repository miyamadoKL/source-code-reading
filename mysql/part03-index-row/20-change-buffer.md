...
第19章で、行の挿入と更新と削除がクラスタ化インデックスとセカンダリインデックスの両方を書き換えることを読んだ。
...
第15章で、バッファプールがページ単位のキャッシュであり、要求されたページがメモリに無ければディスクから読み込むことを読んだ。
チェンジバッファは、この「メモリに無ければ読み込む」という動作のうち、セカンダリインデックスのリーフページへの読み込みを遅延させる仕掛けである。

第17章で、B+tree のリーフページにレコードが格納されることを読んだ。
...
これら3種が、第19章で見た DML の3つの局面に対応する。
...
この二段構えは、第18章と第19章で見た B+tree の楽観的な操作と悲観的な操作の使い分けと同じ考え方である。
...
読み込み完了を処理する `buf_page_io_complete`（第15章）が、セカンダリインデックスのリーフページに対して `ibuf_merge_or_delete_for_page` を呼ぶ。
...
機能としては既定で無効（`innodb_change_buffering = none`）であり、近年の InnoDB では非推奨の方向にある。
コード自体は残っており、明示的に有効化すれば本章で読んだとおりに動作する。

## 関連する章

- [第15章 バッファプール](../part02-innodb-foundation/15-buffer-pool.md)：ページ読み込みの完了時に併合が起動する。
- [第16章 ミニトランザクション](../part02-innodb-foundation/16-mini-transaction.md)：チェンジバッファの記録と併合は専用 mtr の文脈で行われる。
- [第17章 B+tree インデックス](17-btree-index.md)：チェンジバッファ自身も1本の B+tree である。
- [第19章 行の挿入、更新、削除](19-row-dml.md)：本章でバッファする3種の操作は、ここで見た DML の局面に対応する。
- [第25章 undo ログとパージ](../part04-transaction-concurrency/25-undo-and-purge.md)：物理削除のバッファはパージ処理から発行される。
