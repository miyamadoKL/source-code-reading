...
第17章で B+tree のインデックスを、第18章でレコード検索とカーソルを読んだ。
...
第17章で、InnoDB のインデックスがクラスタ化インデックスもセカンダリインデックスも B+tree であることを読んだ。
カーソルがレコードを探す入口は `btr_cur_search_to_nth_level`（第18章）であり、本章の AHI 利用はこの関数の冒頭から呼ばれる。

ページの取得はミニトランザクション（mtr、第16章）の文脈で行う。
AHI が当たると、本来なら mtr に登録するはずだった内部ページのラッチを取らずに、葉ページのラッチだけを取って返す。
ここで言う「ホットなページ」は、第15章のバッファプールに載っているページである。
...
鍵となるのは、ハッシュのキーが「レコード全体」ではなく「インデックスキーの先頭プレフィックス」である点である。
...
- [第17章 B+tree インデックス](17-btree-index.md)：AHI が短絡する対象である B+tree の構造と降下。
- [第18章 レコード検索とカーソル](18-search-and-cursor.md)：AHI の利用と観測を呼び出すカーソル検索の入口。
- [第15章 バッファプール](../part02-innodb-foundation/15-buffer-pool.md)：AHI のエントリが指すレコードを載せるページのキャッシュと、追い出し時のエントリ破棄。
- [第16章 ミニトランザクション](../part02-innodb-foundation/16-mini-transaction.md)：AHI 当たり時に取得する葉ページラッチを登録する mtr。
