...
InnoDB のテーブルもインデックスも、すべてこの1つのデータ構造の上に載っている。
...
行のすべての列、および隠れ列 `DB_TRX_ID` と `DB_ROLL_PTR` は、このクラスタ化インデックスの葉レコードに収まっている。
...
InnoDB のテーブルとインデックスはすべて B+tree であり、クラスタ化インデックスは葉に行本体を主キー順で持ち、セカンダリインデックスは葉にインデックス値と主キー値を持つ。
