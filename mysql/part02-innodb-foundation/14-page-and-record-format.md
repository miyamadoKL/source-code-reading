...
第13章まででテーブルスペースを読み、ファイルが固定長のページに分割されることを確認した。
...
ページをまたいだ木全体の探索とカーソルの動きは第18章へ送り、ここではページ1枚の中で完結する話に絞る。

## 前提

第13章で、テーブルスペースが `FSP_PAGE_SIZE`（既定で16KB）の固定長ページに分かれること、各ページの先頭に FIL ヘッダが置かれることを読んだ。
本章はその FIL ヘッダの内側、つまり INDEX ページ本体の構造を扱う。

レコードの隠れ列に現れる `DB_TRX_ID` と `DB_ROLL_PTR` は、MVCC とトランザクションのための値である。
本章ではこれらがレコードのどこに何バイトで埋め込まれるかだけを読み、それらをたどって過去バージョンを再構成する仕組みは第24章と第25章で扱う。
...
古いリードビューがまだそのレコードを見ている可能性があるため、物理的な回収はパージ処理に委ねる（第25章）。
...
読み手はこの2つをたどって、自分のリードビューに見えるべき過去バージョンへ遡る（第24章）。
...
- [第13章 テーブルスペースとファイル空間管理](13-tablespace-and-fsp.md)：本章のページが置かれるファイル空間と、ページ番号の割り当てを扱う。
- [第15章 バッファプール](15-buffer-pool.md)：本章のページがメモリ上でどのフレームに載り、どう管理されるかを扱う。
- [第17章 B+tree インデックス](../part03-index-row/17-btree-index.md)：本章のページを節点とする木構造と、ページ分割や併合を扱う。
- [第18章 レコード検索とカーソル](../part03-index-row/18-search-and-cursor.md)：本章のページ内二分探索を、木をまたぐ探索とカーソル移動へ拡張する。
- [第24章 MVCC とリードビュー](../part04-transaction-concurrency/24-mvcc-and-read-view.md)：本章の隠れ列 `DB_TRX_ID` と `DB_ROLL_PTR` をたどって過去バージョンを再構成する仕組みを扱う。
