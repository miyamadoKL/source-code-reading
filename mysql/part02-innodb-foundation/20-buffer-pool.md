...
第18章でテーブルスペースを、第19章でページとレコードのフォーマットを読んだ。
これらはすべてディスク上の表現である。
...
ディスクへ書き戻すフラッシュと、それを担う page cleaner スレッドは第33章で読む。
本章はメモリへ載せる側、すなわちページ取得と、空き枠を作るための犠牲ページ選択までを扱う。

## 前提

第19章で、InnoDB のデータがすべて固定長（既定 16 KB）のページに格納されることを読んだ。
バッファプールが扱う単位もこのページである。
ページはテーブルスペース ID とページ番号の組で一意に識別される。
本章ではこの組を `page_id_t` と呼ぶ。

ページの読み書きは **ミニトランザクション**（mtr）の文脈で行われる。
mtr は第21章で読むが、本章では「ページを取得するとき mtr にそのページのラッチを登録し、mtr の終了時にまとめて解放する仕組み」とだけ理解しておけばよい。
...
スキャン中の連続アクセスは時間間隔が短いため、しきい値を越えられず、ページは old に留まったまま追い出し対象になる。
...
このフラッシュ経路の詳細と page cleaner の動作は第33章で読む。
...
- [第18章 テーブルスペースとファイル空間管理](18-tablespace-and-fsp.md)：バッファプールが載せ降ろしするページの、ディスク上の住所を扱う。
- [第19章 ページとレコードのフォーマット](19-page-and-record-format.md)：バッファプールが扱うページの内部構造。
- [第21章 ミニトランザクション](21-mini-transaction.md)：`buf_page_get_gen` が取得したラッチを登録する mtr の仕組み。
- [第33章 ダブルライトバッファとページフラッシュ](../part05-log-recovery/33-doublewrite-and-flush.md)：本章で第33章へ送ったダーティページのフラッシュと page cleaner。
