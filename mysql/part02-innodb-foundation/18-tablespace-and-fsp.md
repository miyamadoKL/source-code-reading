...
第17章で、InnoDB がデータをページ単位で扱い、テーブルスペースというファイルの集合に格納することを概観した。
...
第17章で、InnoDB のページサイズが既定で16KiBであること、各テーブルが専用の `.ibd` テーブルスペースを持つこと（`innodb_file_per_table`）を見た。
本章はその上に立つ。
ページの内部レイアウト（レコードがページにどう並ぶか）は第19章で、B+tree がページをノードとしてどう連結するかは第22章で扱う。
本章が答えるのは、その手前の問いである。
...
これはミニトランザクション（mtr、第21章）に redo ログのレコードを積みながら書く操作である。
...
- [第17章 InnoDB アーキテクチャ概観](17-innodb-architecture.md)：テーブルスペースとページの位置づけを概観した。
- [第19章 ページとレコードのフォーマット](19-page-and-record-format.md)：本章で割り当てたページの内部レイアウトを読む。
- [第20章 バッファプール](20-buffer-pool.md)：`fil_io` を呼んでページを読み込み、キャッシュする層を読む。
- [第21章 ミニトランザクション](21-mini-transaction.md)：本章の `mlog_write_*` が積む redo ログと mtr の仕組みを読む。
- [第22章 B+tree インデックス](../part03-index-row/22-btree-index.md)：本章の2セグメントの上に構築される B+tree を読む。
