...
第15章で読んだバッファプールは、ディスク上のページをメモリへ載せ、変更を遅延させて書き戻す仕組みだった。
...
mtr は、SQL レベルのトランザクション（第23章）とは別の、もっと低い層の概念である。
...
redo ログそのものの構造や、書き出した先でどう永続化されるかは第27章へ送る。

## 前提

第15章で、ページはバッファプール上の `buf_block_t` として保持され、各ブロックが `rw_lock_t` のラッチを持つことを読んだ。
...
mtr が貯めた redo を、グローバルな redo ログバッファのどこへ、どんなレイアウトで書くかは第27章で読む。
...
第15章で読んだ `buf_page_get_gen` 系の取得関数は、要求されたモードでブロックのラッチを取り、そのブロックを mtr のメモへ積む。
...
重要なのは、解放がコミット時に1か所でまとめて起きることである。
...
この順序が守られるからこそ、クラッシュリカバリ（第29章）が成立する。
...
そのバッファをいつディスクへ同期するか、SQL のコミット時にどう待ち合わせるかは第27章の話である。
...
- [第15章 バッファプール](15-buffer-pool.md)：mtr がラッチを取り、ダーティページとしてフラッシュリストへ登録する `buf_block_t` の管理を読む。
- [第17章 B+tree インデックス](../part03-index-row/17-btree-index.md)：複数ページにまたがる挿入や分割を、mtr がどう1単位にまとめるかを読む。
- [第27章 redo ログ](../part05-log-recovery/27-redo-log.md)：mtr が書き出した redo を、ログバッファからどう永続化するかを読む。
- [第29章 チェックポイントとクラッシュリカバリ](../part05-log-recovery/29-checkpoint-and-recovery.md)：mtr の redo を再生して、クラッシュ後にページを復元する仕組みを読む。
