...
第32章では、ページを変更する前にその変更を redo ログへ書く WAL の仕組みを読んだ。
...
LSN（log sequence number）が、redo ログを書いた総バイト数に対応する単調増加の番号であることは第32章で見た。
各ページのヘッダ `FIL_PAGE_LSN` には、そのページに最後に適用された変更の LSN が刻まれる。
バッファプールのダーティページは**フラッシュリスト**につながれ、その先頭にはもっとも古い未フラッシュの変更の LSN（oldest modification）が並ぶことは第20章で見た。
本章ではこのフラッシュリストの最古 LSN を、チェックポイントを進められる上限の根拠として使う。

未コミットトランザクションの巻き戻しに使う undo ログと、その適用については第30章で読んだ。
...
undo の適用そのものは第30章で読んだロールバックと同じ仕組みであり、`DB_ROLL_PTR` をたどって更新前イメージを書き戻す。

ロールバックがバックグラウンドの別スレッドに置かれているのは、redo の再適用が終わればデータベースが論理的に一貫した状態になり、未コミット分の取り消しを待たずに新しい接続を受け付けられるからである。
巻き戻しの対象だったレコードには、まだ undo が残っているので、新しいトランザクションがそれを読んでも MVCC が過去の版を見せる（第29章）。
...
- [第20章 バッファプール](../part02-innodb-foundation/20-buffer-pool.md)：フラッシュリストと oldest modification を扱い、チェックポイントの上限の根拠を与える。
- [第21章 ミニトランザクション](../part02-innodb-foundation/21-mini-transaction.md)：redo を生む単位であり、再適用される変更の粒度を決める。
- [第29章 MVCC とリードビュー](../part04-transaction-concurrency/29-mvcc-and-read-view.md)：巻き戻し対象の版を、新しいトランザクションがどう見るかを扱う。
- [第30章 undo ログとパージ](../part04-transaction-concurrency/30-undo-and-purge.md)：未コミットトランザクションの巻き戻しに使う undo の仕組みを扱う。
- [第32章 redo ログ](32-redo-log.md)：LSN と WAL の基礎を扱い、本章の起点となるログを書く側を読む。
