...
比較の左辺 `oldSize` は「旧テーブルのままヘッダなしで符号化したときのビット数」、右辺 `hSize + newSize` は「新しいテーブル記述を書いたうえで新テーブルで符号化したときのビット数」である。
...
リテラルをどう分割・蓄積するかは第12章、この符号化結果とシーケンス符号化を合わせてブロックに詰める処理は第14章、ブロックをさらに細分する処理は第15章で扱う。
