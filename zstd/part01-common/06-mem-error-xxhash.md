# 第6章 メモリアクセス、エラー表現、XXH64 チェックサム
...
zstd の圧縮と復号の処理は、フレーム内の各フィールドをバイト列から直接読み書きし、失敗を戻り値に埋め込み、末尾のチェックサムで壊れを検出するという3つの下回りの上に成り立つ。
...
また、圧縮と復号の API の多くは戻り値の型に `size_t` を使う。
...
zstd の圧縮と復号の関数の多くは、成功時にはバイト数を、失敗時にはエラーコードを、同じ `size_t` の戻り値で返す。
...
現実の圧縮と復号の処理が実際に扱うバッファサイズがこの値を超えることはまずないため、`ERR_isError` は「戻り値がこの巨大な値より大きいか」だけを見て、エラーかどうかを一つの比較で判定できる。
...
これらの定数は、ビット列に0と1が偏りなく現れるよう選ばれた奇数の素数であり、乗算や回転と組み合わせることで入力のわずかな変化を出力全体へ拡散させる[^avalanche]。
...
`error_private.h` は列挙値を符号反転した `size_t` にエラーを埋め込み、`ERR_isError` の一回の比較で成功か失敗かを判定できるようにしている。
