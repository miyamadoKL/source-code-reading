...
ブロック本体のリテラル・シーケンス復号は第23章に譲り、本章は frame という単位を通す制御に絞る。
...
`inBuff` と `outBuff` は同じ確保領域を前半・後半で分け合う内部バッファであり、呼び出し側の入出力バッファと frame の境界がずれたときの一時置き場になる。
...
- [第6章 メモリアクセス・エラー表現・XXH64チェックサム](../part01-common/06-mem-error-xxhash.md)
