...
しかし zstd の圧縮・復号は、フレームに含まれない共有データとして**辞書**（dictionary）を使える。
...
辞書を初期化する `ZSTD_initDDict_internal` は、辞書内容を確保・コピーしたあと、`ZSTD_loadEntropy_intoDDict` を呼んで辞書の中身を解析する。
...
`ZSTD_DDict` を作る時点で1度だけ計算しておけば、同じ辞書を使う何百・何千回もの復号呼び出しで、そのたびにテーブルを組み立て直す必要がなくなる。
