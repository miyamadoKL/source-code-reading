...
このfastループは、`HUF_DecompressFastArgs_init`で4ストリームぶんの入力と出力のポインタとビットコンテナを組み立て、`HUF_decompress4X1_usingDTable_internal_fast`から呼び出される。
...
`HUF_flags_disableAsm`はこの切り替えを利用者側から止めるための逃げ道であり、`HUF_flags_disableFast`はfastループそのもの（Cループとアセンブラループの両方）を無効化してfallbackだけを使わせるための逃げ道である。
