...
この表が示す主張は2点にまとめられる。
第1に、zstdは`zlib`と同等以上の圧縮率（2.887 対 2.743）を、桁の違う圧縮速度（510 対 95 MB/s）と復号速度（1580 対 400 MB/s）で達成する。
第2に、`--fast`側へ振れば`lz4`や`snappy`に近い速度域まで連続的に移動でき、レベルを上げれば`brotli`級の圧縮率まで届く。
...
`zlib`が土台とするDEFLATEは、リテラルもマッチ長も距離も一律にHuffman符号で表す。
Huffman符号は1シンボルに整数ビット長しか割り当てられないため、理論上の最小ビット長との間に端数ぶんの無駄が残る。
zstdはリテラル長、マッチ長、オフセットのコードをFSEで符号化し、テーブル参照だけで分数ビット長に近い符号長を実現する（第7章、第14章）。
...
単一の探索方式に固定された`lz4`や`snappy`が速度側の一点を、`brotli`や`xz`が圧縮率側の一点を占めるのに対し、zstdが1つの実装で速度と圧縮率の広い曲線を覆えるのは、この2段分離と`strategy`の可変性による。
...
zstdはウィンドウを最大で数百MB規模まで広げられ、さらに**LDM**（Long Distance Matching）が粗い間隔のハッシュ表で長距離のマッチを安価に拾う（第19章）。
