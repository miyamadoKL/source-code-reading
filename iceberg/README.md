...
本書は `format/`、`api/`、`core/` と主要な入出力実装（`parquet/`、`orc/`、`hive-metastore/`）を中心に、Spark/Flink 連携（`spark/`、`flink/`）までを対象とする。
各クラウドストレージ実装（`aws/`、`gcp/`、`azure/` 等）は対象外とする。
...
23. [Spark 連携](part10-engine-integration/23-spark-integration.md)
24. [Flink 連携](part10-engine-integration/24-flink-integration.md)
