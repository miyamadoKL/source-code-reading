...
V3 では位置削除ファイル（Parquet 形式の `(file_path, pos)` ペア）は使用できず、必ず「削除ベクトル」を使う。
...
V2 では「削除ベクトル」を禁止し、V3 以降では「削除ベクトル」を必須とする。
...
V3 では位置削除は「削除ベクトル」のみであるため、「削除ベクトル」が存在する場合は `posDeletesByPartition` と `posDeletesByPath` を検索する必要がない。
...
- Iceberg は行レベルの削除を、データファイルの書き換えなしに実現する。V2 では「位置削除」と「等値削除」、V3 では「削除ベクトル」が使われる
