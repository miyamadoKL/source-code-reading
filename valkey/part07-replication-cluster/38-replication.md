...
主従レプリケーションが、主の書き込みをどうやってレプリカへ届け、両者の状態を一致させ続けるのかを理解する。
...
レプリケーションは、ひとつの主（primary）が受けた書き込みを、複数のレプリカ（replica）へ伝えて同じデータセットを保つ仕組みである。
レプリカは `REPLICAOF <host> <port>` で主を指定し、主に接続して同期を始める。
このコマンドを処理するのが `replicaofCommand` である。

[`src/replication.c` L4609-L4644](https://github.com/valkey-io/valkey/blob/9.1.0/src/replication.c#L4609-L4644)
...
`REPLICAOF NO ONE` を受け取ると `replicationUnsetPrimary` で主への従属をやめ、自身を主に戻す。
それ以外では `replicationSetPrimary` で新しい主の住所を登録し、以後の接続と同期を起動する。
...
自身が主を持ち（`server.primary_host`）、`replica-read-only` が有効で（`server.repl_replica_ro`）、主からの伝播ではない（`!obey_client`）通常クライアントの書き込みは、`-READONLY` エラーで拒否される。
主から流れてきた書き込みだけがレプリカのデータを変える。
この一方向性が、レプリカが主のコピーであり続けることの土台になる。

## レプリケーションのオフセットと replication ID

レプリケーションは、主が生成したコマンドストリームを連続したバイト列とみなし、その先頭からの累積バイト数をオフセットで数える。
主の現在地は `server.primary_repl_offset` が保持する。
...
レプリカは「主のどのオフセットまで受け取ったか」を覚えておき、再接続のときにその続きを要求する。
ただしオフセットだけでは足りない。
別の主のストリームの同じオフセットと取り違えてはならないからである。
そこで主は自分の履歴を表す乱数の識別子 **replication ID**（`server.replid`）を持つ。
...
これにより、過去の主と同じオフセットを示してきたレプリカとの誤った接続を防ぐ。
...
[`src/replication.c` L2090-L2102](https://github.com/valkey-io/valkey/blob/9.1.0/src/replication.c#L2090-L2102)
...
昇格直後のレプリカは、旧主の ID（`replid2`）と当時のオフセットを引き継いでいる。
旧主に従属していた兄弟レプリカが、昇格した新主へ旧 ID で再接続してきても、`second_replid_offset` までなら同じ履歴とみなして部分再同期を受け入れられる。
...
[`src/replication.c` L3490-L3507](https://github.com/valkey-io/valkey/blob/9.1.0/src/replication.c#L3490-L3507)
...
過去に主と話した記録（`cached_primary`）があれば、その replication ID と「まだ受け取っていない最初のバイト」のオフセットを送る。
記録がなければ replid に `?`、オフセットに `-1` を入れる。
これは「履歴を持たないので完全再同期でよい」という明示の合図である。

主の側は `syncCommand` で `SYNC` と `PSYNC` の両方を受ける。
...
要求された replication ID が主の `replid` とも `replid2` とも一致しなければ、履歴が違うので続行できない。

[`src/replication.c` L866-L887](https://github.com/valkey-io/valkey/blob/9.1.0/src/replication.c#L866-L887)
...
両方を満たしたときだけ、主は `+CONTINUE` を返して差分を送る。

[`src/replication.c` L933-L947](https://github.com/valkey-io/valkey/blob/9.1.0/src/replication.c#L933-L947)
...
完全再同期のときは、主は `+FULLRESYNC <replid> <offset>` を返す。
これを送るのが `replicationSetupReplicaForFullResync` である。

[`src/replication.c` L830-L846](https://github.com/valkey-io/valkey/blob/9.1.0/src/replication.c#L830-L846)
...
[`src/replication.c` L3558-L3582](https://github.com/valkey-io/valkey/blob/9.1.0/src/replication.c#L3558-L3582)
...
レプリカは応答から主の replication ID と初期オフセットを取り出し、これを起点に主のコピーを作り直す。
...
部分再同期を成り立たせるのが、主が保持する循環的な履歴バッファ、**レプリケーションバックログ**である。
主はレプリカへ送ったコマンドストリームを、同じ内容でバックログにも書き込む。
...
[`src/replication.c` L464-L508](https://github.com/valkey-io/valkey/blob/9.1.0/src/replication.c#L464-L508)
...
完全再同期では主が RDB スナップショットをレプリカへ送る。
...
[`src/replication.c` L1002-L1022](https://github.com/valkey-io/valkey/blob/9.1.0/src/replication.c#L1002-L1022)
...
主は対象レプリカのソケットを集め、子プロセスを fork して、その子が RDB をソケットへ直接書き出す。
...
完全再同期で初期コピーを作ったあとは、主が受けた書き込みを継続的にレプリカへ流す。
...
自身がレプリカなら（`server.primary_host != NULL`）ここでは伝播せず、主から受けたストリームをそのまま下流へ中継する。
これにより、レプリカのレプリカ（連鎖レプリケーション）でも、主と同一のオフセットと replication ID を保てる。

整形したバイト列は、前述の `feedReplicationBuffer` を通って、接続中の各レプリカとバックログの共有ブロック列に入る。

[`src/replication.c` L606-L628](https://github.com/valkey-io/valkey/blob/9.1.0/src/replication.c#L606-L628)
...
レプリカは受け取ったストリームを適用し、定期的に `REPLCONF ACK <offset>` で「ここまで処理した」を主へ知らせる。
主はこれを `replconfCommand` の `ack` 分岐で受ける。
...
主は各レプリカの確認済みオフセット（`repl_ack_off`）と最終 ACK 時刻（`repl_ack_time`）を更新する。
...
- レプリカは `REPLICAOF` で主に従属し、既定では読み取り専用になる。主から流れてきた書き込みだけがレプリカのデータを変える。
- レプリケーションは、コマンドストリームを連続バイト列とみなしオフセットで数え、履歴の同一性を replication ID で識別する。
- `PSYNC <replid> <offset>` のハンドシェイクで、主は replication ID の一致と要求オフセットがバックログ内かを調べ、満たせば `+CONTINUE`（部分再同期）、満たさなければ `+FULLRESYNC`（完全再同期）に分岐する。
...
- 主は書き込みを `replicationFeedReplicas` で全レプリカとバックログへ流し、レプリカは `REPLCONF ACK` で進捗を返す。`WAIT` はこの ACK を数えて書き込みの到達を確認する。
...
- [第39章 クラスタ](./39-cluster.md)：複数の主従グループでスロットを分担する構成と、その上でのレプリケーション。
