...
Sentinel は、クラスタ機能を使わない主従構成で主の死活を監視し、主が落ちたときにレプリカを自動で昇格させる仕組みである。
...
- レプリケーションの仕組み（主とレプリカの役割、`REPLICAOF`）は[第38章](./38-replication.md)で扱う。本章は、その上で死活監視と昇格を自動化する層を読む。
- Sentinel 同士の通知に使う Pub/Sub の機構は[第43章](../part08-features/43-pubsub.md)で扱う。

## Sentinel が担う四つの仕事

Sentinel は通常の Valkey サーバを `--sentinel` 付きで起動した特別なモードである。
専用ポート 26379 で待ち受け、監視対象の主・レプリカ・他の Sentinel へ定期的にコマンドを送りながら、構成の変化を追い続ける。

その役割は四つに整理できる。

- **監視**：主とレプリカが期待どおり動いているかを定期的に確認する。
- **通知**：障害などの事象を、ログと Pub/Sub チャンネルへ流す。
- **自動フェイルオーバー**：主が落ちたと判断したら、レプリカの一つを新しい主へ昇格させ、残りのレプリカを新主に向け直す。
- **構成提供**：クライアントは Sentinel に問い合わせて、現在どのアドレスが主かを知る。フェイルオーバー後もこの問い合わせ先は変わらない。
...
同じ Sentinel を複数の主から監視している場合、同一相手への接続を一本にまとめる仕掛けである。
...
第二は、主だと思っている相手が自分を「レプリカ」だと報告し続けている場合で、フェイルオーバー後の取り残しなどを検出する。
第三は、主の再起動を検出した場合である。
...
複数の Sentinel が同じ主を SDOWN と見ていることを確かめた状態が **ODOWN**（objectively down、客観的ダウン）である。
...
数え方は明快である。
まず自分自身を 1 票（`quorum = 1`）と数え、知っている他の Sentinel のうち、当の主を落ちていると報告しているもの（`SRI_PRIMARY_DOWN` フラグが立っているもの）を加算する。
...
`quorum` 台の合意が揃って初めてフェイルオーバーへ進むため、単一ノードの誤検知や局所的なネットワーク分断で主が誤って降ろされることを、多数決で防げる。
コメントが「ODOWN は弱い quorum であり、N 台が同じ瞬間に合意したという強い保証はない」と断っているとおり、ODOWN は時間幅の中での合意にすぎない。
それでも、フェイルオーバーを始める前にもう一段の多数決（リーダー選出）を重ねることで、複数の Sentinel が同時に主を昇格させてしまう事態を避ける。
...
これにより、設定に直接書かなくても、同じ主を監視している Sentinel 同士が自動で顔ぶれを把握する。

送信側が `sentinelSendHello` である。
hello のペイロードは、自分（Sentinel）の情報と、現在認識している主の情報をカンマ区切りで並べた 8 トークンになっている。
...
また hello には主の構成エポック（`config_epoch`）が載っており、受け取った構成のほうが新しければ、自分が知る主のアドレスを更新する。
フェイルオーバー後に新しい主のアドレスが Sentinel 群へ伝播するのは、この経路である。
...
主が ODOWN であること、すでにフェイルオーバー中でないこと、直近に試みすぎていないことを確かめる。
...
ODOWN は「主が落ちている」という観測の合意にすぎないのに対し、リーダー選出は「誰が処置を実行するか」を一意に決める。
`sentinel monitor` のコメントが「quorum がいくつであれ、フェイルオーバーを始めるには既知の Sentinel の多数派に選出される必要がある」と述べていたのはこのためである。
多数派の支持を得た一台だけが昇格を進めるので、少数派に分断された側ではフェイルオーバーが進まず、二つの主が並び立つ事態を避けられる。
...
オフセットが大きいレプリカほど主のデータを多く受け取っているため、昇格時のデータ損失が小さくなる。
...
`SEND_REPLICAOF_NOONE` で、選んだレプリカへ `REPLICAOF NO ONE` を送って主へ昇格させる。
...
昇格が `INFO` で確認できると、次は `RECONF_REPLICAS` で残りのレプリカを新しい主へ向け直す。
`sentinelFailoverReconfNextReplica` は、同時に再構成するレプリカ数を `parallel_syncs` で絞りながら、各レプリカへ新主を指す `REPLICAOF` を送る。
...
ここで `sentinelFailoverSwitchToPromotedReplica` が、自分の構成上で主のアドレスを昇格したレプリカのものに差し替え、`+switch-master` 事象を発火する。
...
これを受け取った（あるいは Sentinel に問い合わせた）クライアントは、新しい主のアドレスへ接続を切り替える。
リーダー以外の Sentinel も、昇格した主からの構成エポックがより新しいことを hello で知り、自分の構成を同じアドレスへ更新する。
こうして、構成提供という Sentinel の最後の仕事が果たされる。

## まとめ

- Sentinel は主従構成の監視・通知・自動フェイルオーバー・構成提供を担う。動作はタイマーハンドラの「監視の前半」と「行動の後半」に集約される。
- 死活判定は二段構えである。各 Sentinel が `down-after-milliseconds` の超過で単独に下す SDOWN と、`quorum` 台の合意で確定する ODOWN を分け、単一ノードの誤検知を多数決で防ぐ。
- ODOWN の後、Sentinel 群はエポックごとに一票の投票で実行リーダーを一台だけ選ぶ。絶対多数と `quorum` の両方を満たした一台だけが昇格を進めるので、二つの主が並び立つことを避けられる。
...
- [第38章 レプリケーション](./38-replication.md)：本章が前提とする主従の仕組みと `REPLICAOF`。
