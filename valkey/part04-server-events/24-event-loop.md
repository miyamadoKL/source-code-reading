...
処理はおおよそ次の順に進む。
次のタイマーまでの待ち時間を計算し、その時間を上限に `aeApiPoll` でスリープし、起きたら発火したファイルイベントのハンドラを呼び、最後に時間イベントを処理する。

```mermaid
flowchart TD
    A[aeProcessEvents 開始] --> B[beforeSleep フックを呼ぶ]
    B --> C[usUntilEarliestTimer で<br/>次のタイマーまでの時間を計算]
    C --> D[aeApiPoll でその時間までスリープする<br/>イベント発火で起床]
...
## beforeSleep が「スリープ直前」にまとめる仕事

`ae` の速さを支える2点目は、ポーリングでスリープする直前にフックを差し込んだことである。
`aeProcessEvents` の冒頭で、`aeApiPoll` に入る直前に `beforesleep` が呼ばれることを先に見た。
このフックの狙いは、一回転のあいだに溜まった細かな出力を、スリープする前にまとめて吐き出すことにある。
...
AOF のフラッシュのように、複数の操作を1回のディスク書き込みにまとめたい処理も、同じ「スリープする直前」のタイミングに置かれる。
...
スリープしていたあいだに進んだ時刻を取り込むなど、起床直後に一度だけ要る後始末をここで行う。
...
- `aeProcessEvents` の一回転は、次のタイマーまでの時間を計算し、その時間を上限に `aeApiPoll` でスリープし、発火したファイルイベントのハンドラを呼び、時間イベントを処理する順に進む。
- ポーリングでスリープする直前に呼ばれる `beforeSleep` は、溜まった応答の書き出しや AOF フラッシュをまとめて行い、システムコールの回数を回転ごとに束ねる。`AE_BARRIER` は永続化を挟んでから応答を送る順序を作るために働く。
