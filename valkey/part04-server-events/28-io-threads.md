...
スレッドごとに1本ある `io_threads_mutex[id]` は、そのスレッドをスリープさせるか起こすかを切り替えるためだけに使う。
...
スレッドを起こす操作は、スリープ用のミューテックスを `pthread_mutex_unlock` で解放することで行う。
...
- I/O スレッド数は固定せず、メインスレッドの稼働率と共有 inbox の滞留量に応じて、スリープ用ミューテックスの解放と取得で増減させる。
