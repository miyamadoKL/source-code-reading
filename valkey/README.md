...
Valkey（[valkey-io/valkey](https://github.com/valkey-io/valkey)）のソースコードを読み解き、各コンポーネントが「何のために、どういう処理を行うか」と「高速化と省メモリの工夫」を、ソースコードを引用しながら日本語で解説するドキュメントである。

- **対象バージョン**：9.1.0（Redis OSS 7.2.4 互換のフォーク、コード引用はすべて [`9.1.0` タグ](https://github.com/valkey-io/valkey/tree/9.1.0)に固定）
- **ライセンス**：BSD-3-Clause（引用の方針はリポジトリルートの[引用とライセンス](../README.md#引用とライセンス)を参照）。
- **想定読者**：C と一般的なサーバ/DB の基礎がある中級エンジニア
- **読み方**：低レベルのデータ構造から順に積み上がる構成で、第0部から順に読むことを想定する。
...
> 全52章すべて執筆済み。
> コード引用は `9.1.0` タグ固定の GitHub リンクから該当行を直接参照できる。
