...
読者は第8章で説明した PromQL の型体系（instant vector / range vector / scalar / string / matrix）を理解していること。
...
上位層は `parse.go` でラッパー、Lexer、検査、補助処理を担い、下位層は goyacc 生成の LALR(1) パーサー（`generated_parser.y`）である。
