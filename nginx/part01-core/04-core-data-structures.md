...
先頭がキャッシュラインに揃った領域を、バケットごとに `bucket_size` 単位で切り出している。`bucket_size` がキャッシュラインより大きければバケットは複数キャッシュラインになり得るが、バケット開始位置とサイズをキャッシュライン境界にそろえることで、探索時のキャッシュラインアクセスを最小化している。
