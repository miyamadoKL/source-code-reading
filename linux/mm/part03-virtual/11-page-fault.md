# 第11章 ページフォールトと handle_mm_fault
...
ユーザーアクセスが PTE を欠いたときの **ページフォールト** が `handle_mm_fault` から `__handle_mm_fault` へどう降り、匿名、ファイル、スワップ、THP の分岐に入るかを読む。
...
hugetlb VMA は通常経路を迂回し、それ以外は `__handle_mm_fault` へ進む。
...
## handle_pte_fault：PTE 種別の分岐

PTE が無ければ欠落処理、スワップなら `do_swap_page`、present なら書き込み保護へ進む。
...
`handle_mm_fault` は権限検査後に hugetlb または `__handle_mm_fault` へ分岐する。
`__handle_mm_fault` はページテーブルを降りながら THP を試し、最終的に `handle_pte_fault` が PTE 種別を選ぶ。
匿名は `do_anonymous_page`、ファイルは `do_fault` が典型経路である。
