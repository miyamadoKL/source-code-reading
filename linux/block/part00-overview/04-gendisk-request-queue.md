# 第4章 gendisk、request_queue、request の所有関係
...
> - [`block/blk-mq.c` L4418-L4441](https://github.com/gregkh/linux/blob/v6.18.38/block/blk-mq.c#L4418-L4441)
...
> - [`block/genhd.c` L1306-L1308](https://github.com/gregkh/linux/blob/v6.18.38/block/genhd.c#L1306-L1308)
...
- [第3章](03-queue-limits-bio-split.md) で queue limits を読んでいること。
...
`tag` はドライバが完了を識別する番号であり、第6章で読む sbitmap ベースの割り当てと対応する。
...
`disk_release` は gendisk 解放時に `disk->queue` の参照を落とす。
`GD_OWNS_QUEUE` を立てたドライバでは、ここで `blk_put_queue` が呼ばれ refcount が 0 になればキューも解放される。
...
`blk_put_queue` は `request_queue` の refcount を減らし、0 なら `blk_free_queue` へ進む。
...
bio から request への変換は `blk_mq_submit_bio` が担う。
ctx と hctx の二層キューは第5章で読む。

## 関連する章

- [第3章 queue limits と bio split](03-queue-limits-bio-split.md)
- [第5章 ソフトウェアキューとハードウェアキュー](../part01-blk-mq/05-blk-mq-queues-hctx-ctx.md)
- [第20章 NVMe コントローラのライフサイクル](../part04-nvme-zoned/20-nvme-controller-lifecycle.md)
