...
`tick_broadcast_mask` と `tick_handle_oneshot_broadcast` の流れを読み、[第18章 NO_HZ](18-no-hz.md) の dyntick idle と接続する。

## 前提

- [第16章 tick デバイスと周期 tick](16-tick-device.md) で per-CPU `tick_device` を読んでいること。
- [第12章 clocksource と clockevents](../part02-timer/12-clocksource-clockevents.md) で `clock_event_device` の feature フラグを押さえていること。
