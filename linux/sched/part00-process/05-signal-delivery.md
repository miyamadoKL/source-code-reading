...
ptrace による配送の書き換えは [第7章 ptrace](07-ptrace.md) の担当であり、本章では境界だけ示す。

## 前提

[exit と wait](04-exit-wait.md) を読んでいること。
`try_to_wake_up` による runnable 化は [try_to_wake_up と wakeup の中核](../part01-core/10-try-to-wake-up.md) の担当である。
...
ptrace 下では `ptrace_signal` が番号を書き換えるが、詳細は [第7章 ptrace](07-ptrace.md) で扱う。
