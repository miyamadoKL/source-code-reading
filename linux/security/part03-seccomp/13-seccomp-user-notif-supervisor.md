# 第13章 `SECCOMP_RET_USER_NOTIF` と supervisor API
...
- [第12章：BPF フィルタ検証と実行、キャッシュ](12-seccomp-bpf-verify-run-cache.md)
...
listener FD は `SECCOMP_FILTER_FLAG_NEW_LISTENER` 付きの filter attach で得る（第11章）。
