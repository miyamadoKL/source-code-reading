...
- [第16章：`struct key` と keyring 階層](16-key-keyring-hierarchy.md)
...
`keyctl.c` の syscall 実装は内部で `request_key_and_link` へ委譲する（第18章）。
...
`request_key` syscall は第18章の upcall 機構へ接続する。

## 関連する章

- [第16章：`struct key` と keyring 階層](16-key-keyring-hierarchy.md)
- [`request_key` と key type の概観](18-request-key-types-overview.md)
