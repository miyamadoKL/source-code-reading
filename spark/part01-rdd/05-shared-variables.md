...
Spark のタスクは原則として読み取り専用で動く。タスク間で変数を共有したい場面では、通常のクロージャ変数では各タスクごとにコピーが直列化されて送信される。共有変数（`Broadcast` と `AccumulatorV2`）は、この非効率を解消するための2つの仕組みである。本章ではブロードキャスト変数がビットトレント方式でクラスタに配布される仕組みと、アキュムレータがタスクの結果をドライバへ安全に集約する仕組みを源码から読み取る。
