...
このキャッシュ許可を一時に一クライアントへ絞ることが、後述する一貫性の要になる。
...
cap によるキャッシュ許可の集中管理が、一貫性と性能を同時に成り立たせる仕組みである。
単一のクライアントが inode を占有する間、MDS はそのクライアントにだけ `Fc`/`Fb` を与える。
クライアントは read をローカルキャッシュから返し、write をバッファに溜め、OSD への往復も MDS への問い合わせもせずに済ませられる。
別のクライアントが同じ inode に触れた瞬間、MDS は `Fc`/`Fb` を revoke し、キャッシュのフラッシュと invalidate を強制してから相手へ cap を渡す。
キャッシュを持てるのが常に一者に限られるため、ローカルキャッシュの高速性を保ちながら、複数クライアント間で古いデータが見えることを防げる。
