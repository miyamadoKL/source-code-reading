...
cap は MDS が inode ごとにクライアントへ与える許可であり、書き手がいない間は複数クライアントが読みキャッシュを共有でき、別のクライアントが非互換な cap（書き込みバッファ `Fb` など）を要求したときに revoke してキャッシュを吐き出させる。
...
sequenceDiagram
  ...
  A->>M: open for write, cap 要求
  M-->>A: cap 付与 "Fr Fw Fc Fb"
  Note over A: write をローカルバッファで処理
  B->>M: 同じ inode に書き込み要求 (Fb)
