...
次に、クライアントの write が BlueStore 内部でどう小片と大片に分解され、そのうえで全 blob の転送データ量から deferred write か direct write かを一度だけ選ぶ流れを追う。
...
この切り分けは、割り当て単位より小さい上書きと、単位に整列した大きな書き込みとで、blob の作り方が違うためにある。
小さい端は既存 blob の一部を更新する形になり、大きな中間は新しい割り当て単位をまるごと占める。
ここで分けるのは blob の整え方であって、後段の deferred か direct かの選択は、この小片・大片の別ではなく全 blob 合算の転送データ量で決まる。
...
なぜ転送データ量が小さいときだけ遠回りさせるのか。
割り当て単位に満たない上書きをデバイスへ直に書くと、小さな I/O が個別に発行され、そのつど往復の遅延が乗る。
deferred はこれを、いったんデータを RocksDB のトランザクション（WAL）へ順次書き込みで載せることで隠す。
クライアントへの応答は RocksDB コミットの時点で返せるので、デバイスへの実書き込みは `_deferred_queue` 経由の後始末に回り、まとめて流せる。
逆に、転送データ量が閾値以上の大きな書き込みは、そのまま直に書けば済むので、WAL を二度書きする deferred はかえって無駄になる。
この判定は blob ごとではなく、確保した全 blob の転送データ量 `data_size` を合算して一度だけ下す（コード上のコメントも "We make one decision and apply it to all blobs" と述べる）。
そのため同一 write 内の blob は、すべて deferred かすべて direct のどちらかにそろう。
...
書き込みパスは、割り当て単位の境界で write を小片と大片に分けて blob を整え、`_do_alloc_write` で pextent を確保したうえで、転送データ量が閾値未満なら deferred、以上なら direct を選ぶ。
deferred は小さな転送を RocksDB のログへ載せてまとめ、応答を RocksDB コミットで返して遅延を隠す。
この deferred か direct かの判定は、小片・大片の別ではなく、確保した全 blob の転送データ量 `data_size` を合算して一度だけ下す。
