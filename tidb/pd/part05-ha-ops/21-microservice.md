...
TSO については、`checkTSOService` にも同様の動的切り替えコードが存在するが、v8.5.6 では `IsTSODynamicSwitchingEnabled = false` であり、このパスは実行されない。

[`server/cluster/cluster.go L123`](https://github.com/tikv/pd/blob/v8.5.6/server/cluster/cluster.go#L123)
...
したがって「APIServiceMode」において TSO は常にマイクロサービス側が担い、TSO マイクロサービスのアドレスが取得できなければ `ErrNotFoundTSOAddr` が返る。
Scheduling のようなフォールバックは TSO には存在しない。
...
Scheduling マイクロサービスが見つからない場合は PD 内部でスケジューリングジョブを起動するフォールバックを行う。
一方、TSO マイクロサービスは v8.5.6 ではフォールバックが無効であり、アドレスが取得できなければ `ErrNotFoundTSOAddr` を返す。
