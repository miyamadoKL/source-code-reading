...
`Slots` は `numSlots` 個のスロットを持つ配列である。
各スロットはミューテックスと `nodes` マップを持ち、`nodes[hash]` に該当ハッシュ値の最新 Node を保持する。
...
新しいイベントが追加されると、`Slots.Add` はイベントの各ハッシュ値に対応するスロットをロックし、`nodes[hash]` に既存の Node があれば依存関係として記録する。
依存は同一スロット内の全 Node ではなく、同一ハッシュ値を持つ Node 間でのみ発生する。
