...
状態遷移は次のとおりである。
`Start()` で Span をノードにバインドして Scheduling 状態に移す。
対象ノードの Dispatcher が Working を報告すると `Check` が成功を記録し、`PostFinish` が Replicating に遷移させる。
Removed 報告やノード障害時は `removed` フラグが立ち、`PostFinish` が Span を Absent に戻して再スケジューリングを可能にする。

`Schedule()` は ScheduleDispatcherRequest(Create) メッセージを送信する。
