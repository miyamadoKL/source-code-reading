...
同じスロットを取り合うコマンドは、後で見るように衝突として直列化されるだけである。
...
重複除去は、1つのコマンドが同じスロットを二重に取りにいくのを防ぐ。
...
こうして、同じスロットを要求する複数のコマンドは、待ち行列の順序どおりに1つずつ直列化される。
...
このあいだ、同じスロットを要求する後続コマンドは待ち行列で静止する。
...
キーのハッシュでスロットを取り、同じスロットを要求するコマンドだけを待ち行列で直列化する。
触れるキーが重ならないコマンドどうしは、別々のスロットに並ぶので、latch の待ちで止まらず同時にスナップショット取得と MVCC 実行へ進める。

この設計が並行度を上げるのは、直列化の単位をノード全体ではなくスロット単位に狭めているからである。
...
latch は、キーのハッシュでスロットを引き、同じスロットを要求するコマンドを待ち行列で直列化する軽量ロックである。
