...
要求するハッシュ値が重ならないコマンドどうしは、待ち行列で互いの先頭を妨げないので並行に走れる。
...
latch は、同じハッシュ値を要求する組を直列化することで、その中間を取る。
狙いは同じキーへの操作を直列化することだが、実装上の単位はキーのハッシュ値であり、別キーでもハッシュが衝突すれば同じ組として直列化される。
