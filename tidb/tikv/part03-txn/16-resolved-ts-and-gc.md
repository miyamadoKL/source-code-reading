...
前半は **resolved_ts** であり、各 Region で「今後コミットされるトランザクションの commit_ts は必ずこの時刻より大きい」と言える時刻を追跡する。
...
resolved_ts は、ある Region について「今後発生するコミットの commit_ts は必ず resolved_ts より大きい」という保証を表す時刻である。
言い換えれば、resolved_ts 以下の時刻でのスナップショット読みには、後から新しい版が割り込んでこない。
...
resolved_ts は、未解決ロックの最小 start_ts と PD の現在時刻の小さいほうとして、Region ごとに「今後のコミットの commit_ts は必ずこれより大きい」時刻を追う。
