...
もう1つは、スキーマの変更を `None → DeleteOnly → WriteOnly → WriteReorganization → Public` という多段階の **スキーマ状態**に分解し、全ノードが隣り合う2つの状態にしか同時に存在しないことを保証しながら一段ずつ進める点である。
この段階分けは Google の F1 が示した手法[^f1]に基づく。

代表例として `CREATE INDEX`（`ADD INDEX`）の状態遷移を1つたどる。
インデックスの中身を埋めるバックフィルの実装は第21章へ、スキーマ変更を全ノードへ伝えるリースと domain の仕組みは第22章へ譲る。
...
1回の呼び出しがジョブの状態を1つだけ進め、その変更が全ノードへ行き渡るのを待つ。
...
最後に `updateGlobalVersionAndWaitSynced` で、そのバージョンを全ノードが取り込むのを待つ。
...
owner は `OwnerUpdateGlobalVersion` で最新バージョンを etcd へ公開し、`waitVersionSynced` で全ノードがそのバージョンに追いつくのを待つ。
...
この待ち合わせがオンライン性の要であり、戻り値が返った時点で「全ノードが最新バージョン以上にいる」ことが保証される。
...
リースを使ったバージョンの追従と、その更新を全ノードが受け取る仕組みは第22章で扱う。
ここでは「1状態進めるごとに1バージョン上げ、全ノードの追従を待ってから次へ進む」という骨格を押さえれば足りる。
...
owner は1状態進めるたびにバージョンを上げ、`waitVersionSynced` で全ノードが追いつくのを待ってから次へ進む。
全ノードが追いついた時点でしか次へ進まないため、クラスタ内のノードは最新の状態 `S` か、1つ前の状態 `S-1` のどちらかにしかいない。
...
本来は最大で `2 * lease` のスリープで全ノードの追従を保証できるが、etcd の通知を併用することで、コメントにあるとおり通常は1状態あたり0.5秒未満で次へ進める。
...
flowchart TD
    ...
    VER --> WAIT["waitVersionSynced で全ノードの追従を待つ"]
...
owner はスキーマの変更を多段階の状態に分け、1状態進めるごとにスキーマバージョンを1つ上げ、全ノードがそのバージョンへ追従するのを待ってから次へ進む。
...
- [第22章 PD クライアントと domain](22-pd-client-and-domain.md)：スキーマバージョンをリースで全ノードへ追従させる仕組みと domain を読む。
