...
次の図は、Delta Merge が Delta と Stable から新しい Stable を作り、その途中で GC safe point 以下の不要な版を落とす流れを示す。
...
flowchart TD
    ...
    drop["GC safe point 以下の不要な古い版と削除済み行を落とす"]
...
準備の段では、Delta を Stable に重ねて整列したストリームに `COMPACT` モードのバージョンフィルタをかけ、GC safe point 以下の不要な版を落としながら新しい Stable を書く。
