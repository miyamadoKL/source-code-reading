...
ただし、同じ resource group 内で `MPPQueryId` が最小のクエリ `min_query_id` だけは、ハード上限 `thread_hard_limit` までスレッドを横取りできる。
...
`check_for_new_min_tso` は、自分の `MPPQueryId` が `min_query_id` 以下であり、かつハード上限に収まるなら通すという条件である。
`check_for_not_min_tso` は、稼働中クエリ数が `active_set_soft_limit` 未満であるか自分が既に稼働集合に入っており、かつソフト上限に収まるなら通すという条件である。
どちらかが成り立てば、タスクは稼働集合へ入って `SCHEDULED` になる。

どちらも成り立たないとき、扱いは自分が `min_query_id` かどうかで分かれる。
自分が `min_query_id` のクエリなら、ソフト上限ではなくハード上限まで使えるはずなので、それでも足りないのは異常である。
...
コメントが述べるとおり、`min_query_id` のクエリは完走しきるべき対象であり、ここで枠を取れないなら例外を投げて打ち切る。
...
`MinTSOScheduler` は、同じ resource group 内で `MPPQueryId` の順序に従って最小のクエリを `min_query_id` として選ぶことで、この循環を断つ。
`MPPQueryId` は TiDB が TSO から採番するタイムスタンプを含み大域的に順序付いた値なので、同じクエリのタスクを持つノード間で優先順位が一致する。
`min_query_id` のクエリは、各ノードでソフト上限を越えてハード上限まで枠を確保できる。既に実行中のクエリも、ソフト上限到達時に停止されることはなく完走が許される。
結果として、メモリ過負荷の下でも resource group 内の最小クエリはそのタスクを持つノードで枠を得て完走し、スレッドを返す。
スレッドが返れば次に古いクエリが最小になって完走でき、ハード上限内で枠を取れる限り、少なくとも1本を前進させる。
...
resource group 内で `MPPQueryId` が最小のクエリだけをハード上限まで優先するこの方式は、同じクエリのタスクを持つノード間で優先順位を一致させることでノード間 deadlock を断ち、ハード上限内で枠を取れる限り少なくとも1本のクエリを完走させる。
