...
PostgreSQL 18.4 のソースコードを日本語で読み解くドキュメントである。
対象は安定版リリース **PostgreSQL 18.4**（GitHub タグ `REL_18_4`）。
コード引用はこのタグに固定した GitHub リンクと、実ソースから取ったコードブロックの2点セットで示す。

プロセスモデル、問い合わせの解析と最適化、エグゼキュータ、ストレージとバッファ管理、MVCC とバキューム、インデックス、トランザクションと並行制御、WAL とリカバリまでを、基礎から順に積み上げて読む。
