...
重量ロックを待つプロセスの休眠と起床は `PGPROC` の待ち行列で行うが、その `PGPROC` 自体の管理は第37章で扱う。
...
衝突して待ち行列に入った場合は、`WaitOnLock` でロックが付与されるまで休眠する。
...
flowchart TD
	...
	I -->|"衝突あり"| K["待ち行列に並び WaitOnLock で休眠"]
...
自プロセスの `LOCALLOCK` で保持済みなら即返し、弱いリレーションロックなら fast-path で `PGPROC` に直接記録し、それで取れなければ共有ロックテーブルの `LOCK` と `PROCLOCK` を用意して衝突を判定し、衝突すれば待ち行列で休眠する。
