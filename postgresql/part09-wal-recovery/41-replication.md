...
長い WAL レコードはページ境界で継続レコードに分割されているため、ページ境界は必ずレコードの切れ目になる。
ここで切ることで、1つの WAL レコードが2つのメッセージにまたがらないことが保証され、受け手はメッセージ境界を気にせず WAL を書ける。
...
`XLogSendPhysical` は1メッセージの末尾を WAL ページ境界へ切り下げ、1つの WAL レコードがメッセージ境界をまたがないことを保証する。
