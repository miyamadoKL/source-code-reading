...
待機するプロセスはロックを取れるまでビジーループで回り、眠らない。
...
`perform_spin_delay` は、まず `SPIN_DELAY()` で CPU 固有の短い待機を入れ、一定回数空転したら `pg_usleep` で実際に眠る。
眠るたびに遅延時間を 1 ミリ秒からおよそ 1 秒まで指数的に伸ばし、上限を超えたら最小値へ戻す。
そして `NUM_DELAYS` 回（18.4 では 1000 回）眠っても取れなければ、`s_lock_stuck` を呼んで PANIC する。
...
待機の流れは「アトミックなテストアンドセット、空転、眠る、最終的に行き詰まりとして PANIC」という段階を踏む。
...
保持者が走らない間、待機者は CPU 上で空転し続け、いずれ眠る。
...
このコメントは、スケジューラによって優先度を下げられた（nice された）保持者が、待機者が全員眠るまでスケジュールされず、そのため飢餓（starvation）が起こりうる、と述べている。
眠る遅延を指数的に増やすのは、待機者が早く眠ることで保持者に CPU を譲り、保持者が走って解放できるようにするためである。
...
すべてのバックエンドが新しいバッファを得るときにこの経路を通るため、コア数が増えるほど取得頻度が上がり、最も熱いロックになる。
...
PostgreSQL のスピンロックは、保持者が数命令で臨界区間を走り抜け、その間プリエンプトされない、という前提に立つ。
この前提は `spin.h` の規約と `README` の位置づけに明文化され、待機ループの実装は保持者がすぐ解放することに賭けてコストを先送りしている。
