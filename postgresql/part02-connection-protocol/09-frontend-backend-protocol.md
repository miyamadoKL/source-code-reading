...
第7章で、バックエンドが入力を待つ間にスリープし、シグナルで安全に起こされる仕組みを読んだ。
...
`ReadCommand` が次のメッセージを読み終えるまで、ループはこの行でスリープする。
...
- [第7章 ラッチとシグナル処理](../part01-process-memory/07-latches-and-signals.md)：メインループが入力待ちでスリープし、シグナルで起こされる仕組み。
