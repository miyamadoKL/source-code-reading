...
- Trino の Connector SPI（`ConnectorMetadata`, `ConnectorSplitManager`, `ConnectorPageSourceProvider`）の役割を知っていること（第21章）。
- Split がデータ読み取りの並列単位であることを理解していること（第12章）。
- Page と Block のデータモデルを知っていること（第19章）。
- Hive Connector の基本構造に馴染みがあること（第22章）。
...
Iceberg Connector（第23章）と同様に、Guice の `Bootstrap` で依存関係を組み立てる。
...
- [第12章 Stage と Task のスケジューリング](../part03-execution/12-stage-and-task-scheduling.md)：Split の分配と並列実行の仕組み。
- [第19章 Page と Block のデータモデル](../part04-data/19-page-and-block.md)：列指向データ表現の内部構造。
- [第21章 Connector SPI の詳細](21-connector-spi-detail.md)：Connector SPI のインターフェース設計。
- [第22章 Hive Connector](22-hive-connector.md)：Hive Metastore と Parquet 読み取りの共通基盤。
- [第23章 Iceberg Connector](23-iceberg-connector.md)：同じレイクハウスフォーマットである Iceberg との設計比較。
