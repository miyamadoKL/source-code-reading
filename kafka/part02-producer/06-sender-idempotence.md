...
冪等性だけを有効にした場合、`TransactionManager`はトランザクション用の状態遷移を素通りし、producerId とエポックとシーケンス番号の管理だけを担う。
...
`accumulator.drain`が呼ばれた時点で、各バッチには producerId とエポックとシーケンス番号が割り当てられる。
...
パーティションごとの応答を受け取る`completeBatch`（引数4つの版）が、実際に成功、リトライ、失敗のどれになるかを判断する本体である。
...
- ブローカー（`UnifiedLog`）は producerId とエポックとパーティションの組ごとに、直近の重複バッチのメタデータを既定で5件保持する。
  同じシーケンス番号のバッチが再度届いた場合、物理的な追記はせず、保持していた元の`appendInfo`（正しい firstOffset と lastOffset）をそのまま返す。
  `DUPLICATE_SEQUENCE_NUMBER`エラーが返るのは、ブローカーがその重複バッチのメタデータをすでに保持していない場合に限られる（第12章の`findDuplicateBatch`を参照）。
- パイプライン化のために複数バッチを同時に送っても、シーケンス番号は`drain`時点でパーティション単位の単調増加として払い出される。
  ブローカーは受理順を並べ替えず、そのシーケンス番号どおりに書き込む。
  応答が送信順と異なる順序で届いた場合の処理は、`isNextSequence`によるクライアント側のリトライやエポック繰り上げが担う。
...
最後に、`guaranteeMessageOrder`が有効なときにミュートしていたパーティションは、成功、リトライ、失敗のどの経路でも`completeBatch`（引数5つの版）の末尾で解除される。
...
冪等プロデューサーは、producerId とエポックと、パーティションごとのシーケンス番号という3つ組をバッチに刻み込むことで、ブローカー側での重複検出を可能にする。
...
- 第12章 [../part03-storage/12-producer-state.md](../part03-storage/12-producer-state.md)（ブローカー側の producerId とシーケンス番号の永続化）
