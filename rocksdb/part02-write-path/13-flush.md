...
[`db/memtable_list.cc` L410-L463](https://github.com/facebook/rocksdb/blob/v11.1.1/db/memtable_list.cc#L410-L463)
...
- フラッシュは満杯になった MemTable を L0 SST へ書き出し、メモリを解放しつつ内容を永続化する。
  書き込みパスの終着点である。
- `SwitchMemtable` が可変 MemTable を Immutable 化し、新しい可変 MemTable と WAL を用意する。
  Immutable は `MemTableList` に積まれる。
- `FlushJob` は `PickMemTable` で対象を選び、`Run` から `WriteLevel0Table` を呼んで MemTable を走査し、`BuildTable` で L0 SST を一つ作る。
- 結果は `TryInstallMemtableFlushResults` が `VersionEdit` にして MANIFEST へ記録する。
  書き出しは並行に走ってよいが、MANIFEST への記録は作成順（FIFO）に固定される。
