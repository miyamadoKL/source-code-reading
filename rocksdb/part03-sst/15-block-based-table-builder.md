...
- [第13章 フラッシュ](../part02-write-path/13-flush.md)：MemTable を SST に書き出す経路。`TableBuilder` の呼び出し元の一つ。
...
[`table/block_based/block_based_table_builder.h` L55-L78](https://github.com/facebook/rocksdb/blob/v11.1.1/table/block_based/block_based_table_builder.h#L55-L78)
...
[`table/block_based/block_based_table_builder.cc` L1816-L1832](https://github.com/facebook/rocksdb/blob/v11.1.1/table/block_based/block_based_table_builder.cc#L1816-L1832)
...
[`table/block_based/block_based_table_builder.cc` L2128-L2160](https://github.com/facebook/rocksdb/blob/v11.1.1/table/block_based/block_based_table_builder.cc#L2128-L2160)
...
[`table/block_based/block_based_table_builder.cc` L2811-L2846](https://github.com/facebook/rocksdb/blob/v11.1.1/table/block_based/block_based_table_builder.cc#L2811-L2846)
...
- `TableBuilder` はソート済みの内部キーと値の列を受け取り、SST のバイト列を組み立てる。フラッシュとコンパクションが呼び出し元になる。
...
- `block_restart_interval` 個ごとにリスタートポイントを置き、そこではキー全体を書き直す。ブロック末尾のリスタート配列がブロック内二分探索の起点になり、圧縮率と探索速度を一つのパラメータで調整できる。
