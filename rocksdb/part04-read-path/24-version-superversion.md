...
[`db/version_set.cc` L4491-L4516](https://github.com/facebook/rocksdb/blob/v11.1.1/db/version_set.cc#L4491-L4516)
...
[`db/version_set.cc` L305-L325](https://github.com/facebook/rocksdb/blob/v11.1.1/db/version_set.cc#L305-L325)
...
[`db/file_indexer.h` L32-L42](https://github.com/facebook/rocksdb/blob/v11.1.1/db/file_indexer.h#L32-L42)
...
[`db/file_indexer.h` L71-L83](https://github.com/facebook/rocksdb/blob/v11.1.1/db/file_indexer.h#L71-L83)
...
[`db/file_indexer.cc` L95-L137](https://github.com/facebook/rocksdb/blob/v11.1.1/db/file_indexer.cc#L95-L137)
...
[`db/file_indexer.cc` L154-L170](https://github.com/facebook/rocksdb/blob/v11.1.1/db/file_indexer.cc#L154-L170)
...
[`db/column_family.cc` L1469-L1484](https://github.com/facebook/rocksdb/blob/v11.1.1/db/column_family.cc#L1469-L1484)
...
- `Version` は `VersionStorageInfo` を通じてレベルごとの SST 集合を持つ。
  L1 以降は各レベル内でキー範囲が重ならずソート済みという不変条件があり、L0 だけは範囲が重なりうるためエポック降順に並ぶ。
...
- `FileIndexer` は `Version` 構築時に各レベルの境界を事前計算し、上のレベルの比較結果を下のレベルの探索区間に持ち越す。
  これによりレベルごとにゼロから二分探索し直す無駄を省き、レベル L で見るファイル数を N^L から N 程度に抑える。
- `SuperVersion` は MemTable、Immutable MemTable、`Version` を同一時点の一組として束ね、参照カウントで寿命を管理する。
- 読み出しはスレッドローカルにキャッシュした `SuperVersion` をアトミック操作だけで取得し、`SuperVersion` が変わらない限り DB ミューテックスを避ける。
  差し替え時は `Scrape` でキャッシュに `kSVObsolete` を置き、次の取得で参照を取り直させる。
