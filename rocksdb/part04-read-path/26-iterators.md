...
- [第17章 インデックスブロック](../part03-sst/17-index-block.md)：SST 内のインデックスとデータブロックの構造。
  two-level イテレータの土台となる。
...
[`include/rocksdb/iterator_base.h` L26-L54](https://github.com/facebook/rocksdb/blob/v11.1.1/include/rocksdb/iterator_base.h#L26-L54)
...
- **`DBIter`**（最上層、`Iterator` の実装）：内部キー列を利用者キーの視点へ変換する。
  可視版の選択、古い版とトゥームストーンの除去、Merge の畳み込み、前後方向の切り替えを担う。
- **`MergingIterator`**（中間層、`InternalIterator` の実装）：複数の子イテレータを内部キー順に併合し、単一のソート列に見せる。
- **各ソースの `InternalIterator`**（最下層）：MemTable のイテレータと、各 SST の two-level イテレータ。
  それぞれが一つのソートされた走査列を供給する。
...
[`table/merging_iterator.cc` L1320-L1384](https://github.com/facebook/rocksdb/blob/v11.1.1/table/merging_iterator.cc#L1320-L1384)
...
[`db/db_iter.h` L35-L45](https://github.com/facebook/rocksdb/blob/v11.1.1/db/db_iter.h#L35-L45)
...
[`db/db_iter.cc` L441-L478](https://github.com/facebook/rocksdb/blob/v11.1.1/db/db_iter.cc#L441-L478)
...
[`db/db_iter.cc` L479-L525](https://github.com/facebook/rocksdb/blob/v11.1.1/db/db_iter.cc#L479-L525)
...
- `MergingIterator` は子イテレータを最小ヒープ（後退時は最大ヒープ）で k-way 併合し、一件あたりの比較を O(log n) に抑える。
  これが本章の最適化の核である。
- `DBIter` は内部キー列を利用者キーへ変換する。
  `IsVisible` でスナップショット以下の版だけを採用し、`FindNextUserEntry` で古い版とトゥームストーンを飛ばし、`MergeValuesNewToOld` で Merge を畳み込む。
